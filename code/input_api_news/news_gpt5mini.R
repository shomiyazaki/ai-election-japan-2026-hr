# -------------------------------------------------------------
# News source classification experiment — GPT-5 Mini
# Web search on/off, 100 iterations each, 4 parallel workers
# Model settings follow 1100_gpt5mini_api_japan_2026_election.R
# Output: data-out/news_source/raw_gpt5mini.rds
...
results <- future_map_dfr(seq_len(n_iter), function(i) {
  ...
  bind_rows(
    tibble(model = "GPT-5 Mini", web_search = TRUE,
           iteration = i, url_order = list(shuffled),
           raw_response = call_web(prompt)),
    tibble(model = "GPT-5 Mini", web_search = FALSE,
           iteration = i, url_order = list(shuffled),
...
saveRDS(results, "data-out/news_source/raw_gpt5mini.rds")
cat("Saved: data-out/news_source/raw_gpt5mini.rds\n")
