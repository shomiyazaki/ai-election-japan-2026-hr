...
# Gemini 3.1 Pro Preview - WITH web search (Google Search grounding)
...
# 1. Settings --------------------------------------------------
google_api_key <- trimws(readLines("google_api_key_3.txt", n = 1, warn = FALSE))
...
get_gemini_response <- function(prompt, model = "gemini-3.1-pro-preview") {
  ...
  # when Google Search grounding interleaves search calls with text)
  ans <- tryCatch({
    parts <- parsed$candidates[[1]]$content$parts
    texts <- unlist(lapply(parts, function(p) if (!is.null(p$text)) p$text else NULL))
    if (length(texts) == 0) NULL else paste(texts, collapse = "\n")
...
outfile <- glue("data-out/flagship_models/japan2026_election_gemini31pro_web_wave_{wave_date}.csv")
