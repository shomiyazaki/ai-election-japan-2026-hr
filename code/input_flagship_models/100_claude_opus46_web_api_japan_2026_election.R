...
# Claude Opus 4.6 - WITH web search (web_search_20260209)
...
# 1. Settings --------------------------------------------------
anthropic_api_key <- trimws(readLines("anthropic_api_key.txt", n = 1, warn = FALSE))
...
# 3. API function -----------------------------------------------
# Anthropic Messages API — response content is a list of blocks;
# filter for type == "text" and concatenate.
get_claude_response <- function(prompt, model = "claude-opus-4-6", temperature = 1.0) {
  res <- NULL
  for (attempt in 1:3) {
    res <- tryCatch({
      POST(
        ...
        body = toJSON(list(
          ...
          tools       = list(list(type = "web_search_20260209", name = "web_search"))
        ), auto_unbox = TRUE),
        timeout(180)
      )
  ...
  if (!is.null(parsed$error)) return(paste("API_ERROR:", parsed$error$message))

  # Extract only text-type content blocks
  text_blocks <- Filter(function(x) !is.null(x$type) && x$type == "text", parsed$content)
  if (length(text_blocks) == 0) return("EMPTY_RESPONSE")

  texts <- sapply(text_blocks, function(x) if (!is.null(x$text)) x$text else "")
...
results <- actors %>%
  mutate(claude_response = future_map_chr(
    prompt,
    ~ get_claude_response(.x, temperature = temperature),
    .progress = TRUE
  )) %>%
...
outfile <- glue("data-out/flagship_models/japan2026_election_claude_opus46_web_wave_{wave_date}.csv")
...
cat("\nSample output:\n\n")
first_valid <- results$claude_response[which(!grepl("^(CONNECTION|API|EMPTY)_", results$claude_response))[1]]
