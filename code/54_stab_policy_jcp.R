# =============================================================================
# 55_stab_policy_jcp.R
# Supplementary: Full policy treatment coefficient table for JCP.
#
# Specification (Eq. 2 in paper):
#   JCP ~ policy_treatment | gender + area_type + wave + region
# FE absorbed: gender, area type, date, district.
# HC1 robust standard errors.
#
# Output: draft/figures/stab_policy_jcp.tex   (Table S5)
# =============================================================================
...
# =============================================================================
# Regressions: JCP ~ policy_treatment | gender + area_type + wave + region
...
models_jcp <- map(set_names(model_levels, table_labels), \(m) {
  feols(JCP ~ policy_treatment | gender + area_type + wave + region,
...
writeLines(c(
  "\\begin{table}[p]",
  "\\caption{Policy treatment coefficients for Pr(JCP) by model.}",
  "\\label{stab:policy-jcp}",
  "\\centering",
  "\\footnotesize",
  "\\resizebox{\\textwidth}{!}{",
  tex_lines,
  "}",
  "\\end{table}"
), file.path(out_fig_dir, "stab_policy_jcp.tex"))

cat("Saved: figures/stab_policy_jcp.tex\n")
